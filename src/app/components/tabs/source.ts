...
interface TabProps {
  ...
  getCurrentTab?: (id: string) => void;
  onTabChange?: (id: string) => void;
}

const TabIndicatorTransition: Transition = {
  type: "spring",
  stiffness: 500,
  damping: 30,
};
...
const Tabs: React.FC<TabProps> = (props) => {
  ...
  const onTabSwitch = (e: React.MouseEvent<HTMLButtonElement>, id: string) => {
  ...
  return (
    <>
      ...
      >
        <ul className="tabs flex-1">
          {filteredTabs.map((tab) => (
            <li
              key={tab.id}
              className={twMerge(
                "relative z-0",
                activeTab === tab.id ? "active-tab" : "tab",
              )}
            >
              <button
                id={tab.id}
                onClick={(e) => {
                  onTabSwitch(e, tab.id);
                }}
              >
                {tab.label}
              </button>
              {activeTab === tab.id && (
                <motion.span
                  ...
                  className={twMerge(
                    "absolute inset-0 -z-10 bg-current",
                    tabIndicatorClassName,
                  )}
            ...
            {filteredTabs.map((tab) => {
              ...
              return (
                <li
                  key={tab.id}
                  className={twMerge(
                    "relative z-0 flex aspect-square h-full max-h-12 w-full max-w-12 items-center justify-center rounded-full fill-current p-2",
                    activeTab === tab.id ? "stroke-bg" : "stroke-current",
                  )}
                >
                  ...
                  {activeTab === tab.id && (
                    <motion.span
                      ...
                      className={twMerge(
                        "absolute inset-0 -z-10 rounded-full bg-current",
                        tabIndicatorClassName,
                      )}
                      transition={TabIndicatorTransition}
                    />
